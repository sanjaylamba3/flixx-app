const global = {
    ...
    api: {
        apiKey: `6ca39d5a6c44e0f389358cbf04ac1ece`,
...
// Diplay 20 most popular TV shows
          ...
          <div class="card-body">
            ...
            <p class="card-text">
              <small class="text-muted">${
                  global.search.type === "movie" ? `Release: ${result.release_date}` : `First Air Date: ${result.first_air_date}`
              }</small>
            </p>
          </div>`;
        document.querySelector("#search-results-heading").innerHTML = `
...
const fetchAPIData = async (endpoint) => {
    ...
    showSpinner();
    const response = await fetch(`${API_URL}${endpoint}?api_key=${API_KEY}&languageg=en-US`);
...
const searchAPIData = async () => {
    ...
    const response = await fetch(
        `${API_URL}search/${global.search.type}?api_key=${API_KEY}&languageg=en-US&query=${global.search.term}&page=${global.search.page}`
    );
    const data = await response.json();
    hideSpinner();
    return data;
